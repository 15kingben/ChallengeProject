    ...
    if (in.is_open()) {
        ...
        while (in) {
            if ((str.find_first_not_of(" \t\r\n") != string::npos) && (str[0] != '#')) {
                ...
                } else if (cmd == "depth") { // NEW
					validinput = readvals(s,1,values);
					if (validinput) {
						float maxdepth = values[0];
					}
				} else if (cmd == "output" ){
					validinput = readvals(s,1,values);
					if (validinput) {
						//std::string output = values[0];
						...
						//syntax: look from x,y,z, look at x,y,z up x,y,z fovy
						eyeinit.x = values[0];
						eyeinit.y = values[1];
						eyeinit.z = values[2];
                ...
                // Also look at demo.txt to get a sense of why things are done this way.
                    ...
                    if (validinput) {
						...
						//goes at the end because last in is first applied
